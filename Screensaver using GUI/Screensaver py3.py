...
width = int(input("enter frame size: "))   
height = int(input("enter frame size: "))  
position = [100,100]
message  = "Hello"
interval = 1000
...
def tick():
    ...
    ''' 
    x = random.randrange(0,width-15*len(message))   # width-10*len(message) to avoid text out-of frame
...
def draw_canvas(canvas):
    ...
    #canvas.draw_circle(position,15*len(message),1,"White")
